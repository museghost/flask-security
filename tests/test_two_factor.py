...
def test_two_factor_flag(app, client):
    # trying to verify code without going
    # through two factor first login function
    wrong_code = '000000'
    response = client.post('/two_factor_token_validation/',
                           data=dict(code=wrong_code),
                           follow_redirects=True)
    assert 'You currently do not have permissions to access this page' \
           in str(response.data)

    # Test login using invalid email
    data = dict(email="nobody@lp.com", password="password")
    response = client.post('/login', data=data, follow_redirects=True)
    assert 'Specified user does not exist' in str(response.data)
    ...
    response = client.post('/login', data=data, follow_redirects=True)
    assert 'Two-factor authentication adds an extra layer of security' \
           in str(response.data)
    response = client.post('/two_factor_setup_function/',
                           data=dict(setup="not_a_method"),
                           follow_redirects=True)
    assert 'Marked method is not valid' in str(response.data)
    ...
    response = client.post('/two_factor_setup_function/',
                           ...
                           headers={'Content-Type': 'application/json'},
                           follow_redirects=True)
    assert '"response": {}' in str(response.data)
    ...
    response = client.post('/login', data=json_data,
                           headers={'Content-Type': 'application/json'},
                           follow_redirects=True)
    assert 'Please enter your authentication code' in str(response.data)
    ...
    response = client.post('/two_factor_token_validation/',
                           data=dict(code=code),
                           follow_redirects=True)
    assert 'Your token has been confirmed' in str(response.data)
    ...
    assert 'You successfully confirmed password' in str(response.data)
    assert 'Two-factor authentication adds an extra layer of security' \
           in str(response.data)

    # change method (from sms to mail)
    setup_data = dict(setup='mail')
    mail_tpl = MailTemplate()
    app.extensions['mail'] = mail_tpl
    response = client.post('/two_factor_setup_function/',
                           data=setup_data,
                           follow_redirects=True)
    assert 'To complete logging in, please enter the code sent to your mail' \
           in str(response.data)

    code = mail_tpl.msg.body.split()[-1]
    # sumbit right token and show appropriate response
    response = client.post('/two_factor_token_validation/',
                           data=dict(code=code),
                           follow_redirects=True)
    assert 'You successfully changed your two factor method' \
           in str(response.data)
    ...
    response = client.post('/two_factor_setup_function/',
                           data=setup_data,
                           follow_redirects=True)
    assert 'Open Google Authenticator on your device' \
           in str(response.data)
    qrcode_page_response = client.get('/two_factor_qrcode/',
                                      data=setup_data,
                                      follow_redirects=True)
    assert 'svg' in str(qrcode_page_response.data)
    ...
    setup_data = dict(setup='google_authenticator')
    response = client.post('/two_factor_setup_function/', data=setup_data,
                           follow_redirects=True)
    assert 'Open Google Authenticator on your device' in str(response.data)
    qrcode_page_response = client.get('/two_factor_qrcode/',
                                      data=setup_data,
                                      follow_redirects=True)
    assert 'svg' in str(qrcode_page_response.data)
    ...
    assert 'To Which Phone Number Should We Send Code To' in str(response.data)
    ...
    response = client.post('/two_factor_rescue_function/',
                           data=rescue_data,
                           follow_redirects=True)
    assert 'The code for authentication was sent to your email address' \
           in str(response.data)
    rescue_data = dict(help_setup='no_mail_access')
    response = client.post('/two_factor_rescue_function/',
                           data=rescue_data,
                           follow_redirects=True)
    assert 'A mail was sent to us in order to reset your application account' \
           in str(response.data)
