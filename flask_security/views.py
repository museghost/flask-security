...
def two_factor_setup_function():
    ...
    # user's email&password not approved or we are logged in and
    # didn't validate password
    ...
    if form.validate_on_submit():
        # totp and primarty_method are added to session to flag
        # the user's temporary choice
...
def two_factor_token_validation():
    ...
    # make sure user has or has chosen a two factor method
...
def two_factor_rescue_function():
    """ Function that handles a situation where user can't enter
    his two factor validation code"""
    ...
    # user's email&password approved and two factor properties
    # were not configured
    if any(['totp_secret' not in session, 'primary_method' not in session]):
        do_flash(*get_message('TWO_FACTOR_PERMISSION_DENIED'))
        return abort(404)
    ...
    if form.validate_on_submit():
        ...
        # if the problem is that user can't access his device,
        # we send him code through mail
...
def create_blueprint(state, import_name):
    ...
    if state.passwordless:
        ...
        bp.route(
            '/' + slash_url_suffix('/', 'two_factor_setup_function'),
            methods=['GET', 'POST'],
            endpoint='two_factor_setup_function')(two_factor_setup_function)
        bp.route(
            '/' + slash_url_suffix('/', 'two_factor_token_validation'),
            methods=['GET', 'POST'],
        ...
        bp.route(
            '/' + slash_url_suffix('/', 'two_factor_rescue_function'),
            methods=['GET', 'POST'],
            endpoint='two_factor_rescue_function')(two_factor_rescue_function)
        bp.route(
            state.change_url + slash_url_suffix(
                state.change_url, 'two_factor_password_confirmation'),
            methods=['GET', 'POST'],
            endpoint='two_factor_password_confirmation')(
